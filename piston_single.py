...
def process_single_tdms_file(tdms_file_path, output_path):
    try:
        logging.info(f"Processing single TDMS file: {tdms_file_path}")
        
        tdms_file = TdmsFile.read(tdms_file_path)
        df = tdms_file.as_dataframe()
        
        df.columns = df.columns.str.replace("/", "").str.replace("'", "").str.replace("Data", "").str.strip()
        
        df['Displacement'] = calculate_pump_displacement(df['Main_Flow_GPM'], df['RPM'])
        
        max_derived_displacement = df['Displacement'].max()
        
        df['Calc_VE'] = df['Displacement'] * 100 / max_derived_displacement
        df['Calc_ME'] = 100 * ((((max_derived_displacement / 16.387064) * df['Outlet_Pressure_Psi']) / (2 * math.pi)) / df['Pump_Torque_In.lbf'])
        df['Calc_OE'] = df['Calc_VE'] * df['Calc_ME'] / 100
        
        os.makedirs(output_path, exist_ok=True)
        df.to_csv(os.path.join(output_path, 'single_file_processed_data.csv'), index=False)
        
        plot_paths = create_single_file_plots(df, output_path)
        
        max_efficiency = df['Calc_OE'].max()
        
        return df, max_derived_displacement, max_efficiency, plot_paths
    except Exception as e:
        logging.exception(f"Error processing single TDMS file: {e}")
        return None, None, None, None

def create_single_file_plots(df, output_path):
    ...
    for column in columns_to_plot:
        try:
            ...
            ax.legend()
            plt.tight_layout()
            plot_path = os.path.join(output_path, f'single_file_{column}_plot.png')
            plt.savefig(plot_path)
            plt.close()
            plot_paths[column] = plot_path
        except Exception as e:
            logging.warning(f"Failed to create plot for {column}: {str(e)}")
            plt.close()  # Close the figure in case of an error
    
    # Create efficiency plots
    try:
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        fig.suptitle("Efficiency Plots")
        
        efficiency_cols = ['Calc_VE', 'Calc_ME', 'Calc_OE']
        ...
        plt.tight_layout()
        efficiency_plot_path = os.path.join(output_path, 'single_file_efficiency_plots.png')
        plt.savefig(efficiency_plot_path)
        plt.close()
        plot_paths['efficiency'] = efficiency_plot_path
    except Exception as e:
        logging.warning(f"Failed to create efficiency plots: {str(e)}")
        plt.close()  # Close the figure in case of an error
