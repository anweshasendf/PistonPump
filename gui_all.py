...
# If you want to disable all matplotlib-related logging
...
class DisplayWindow(QMainWindow):
    ...
    def read_max_derived_displacement(self):
        ...
        try:
            ...
            logging.info(f"Read max_derived_displacement: {max_derived_displacement}")
            return max_derived_displacement
        except Exception as e:
            logging.error(f"Error reading max_derived_displacement: {e}")
            return None
    ...
    def create_contour_plot(self, df, rpm_col, pressure_col, efficiency_col, title, filename):
        ...
        canvas = FigureCanvasQTAgg(fig)
        # Resize the figure to fit the GUI window
        canvas.setFixedSize(self.width(), self.height())
        canvas.draw()
        ...
        # Add the plot widget to a new tab
        tab = QWidget()
        tab_layout = QVBoxLayout(tab)
